...
def remove_duplicate(usr_id, min_occurrence=5000, min_tag=3):
    print 'Sorting user by time...'
    usr_list = [f for f in os.listdir(time_path) if f.endswith('_'+usr_id+'.txt') and f.startswith('test')]
    for usr in usr_list:
        print usr, '...'
...
def split_event(usr_id, min_picture_event = 30, time_diff = 3600*3, path=clean_path, save_path =event_path):
    ...
    out_file.close()
def test(usr_id, i):
    load_path = root + 'clean_tags_5000_3/'+i+'.txt'
    save_path = root + 'clean_tags_5000_3/test_'+i+'_'+usr_id+'.txt'
    count_usr = 0
    f = open(save_path,'w')
...
if __name__ == '__main__':
    ...
    for j in xrange(10):
        ...
        i = str(j)
        test(usr_id, i)
        sort_usr_with_time(['test_'+i+'_' + usr_id + '.txt'])
