...
def init_params(options):
    ...
    params = OrderedDict()
    # embedding
    params['Wemb'] = (numpy.load(root + 'feature_all.npy')).astype(config.floatX)
...
def train_lstm(
    ...
    with open(root + 'training_imdb.pkl') as f:
        train = pickle.load(f)
    with open(root + 'test_imdb.pkl') as f:
        test = pickle.load(f)
    with open(root + 'test_imdb.pkl') as f:
        valid = pickle.load(f)
    valid_sample_idx = random.sample(range(len(valid)), 50)
        ...
        for eidx in range(max_epochs):
            ...
            for _, train_index in kf:
                ...
                if saveto and numpy.mod(uidx, saveFreq) == 0:
                    ...
                    numpy.savez(saveto + str(uidx) + '.npz', history_errs=history_errs, **params)
    ...
    if saveto:
        numpy.savez(saveto + '.npz', train_err=train_err,
...
def test_prediction(
    ...
    params = init_params(model_options)
    load_params('lstm_model588.npz', params)
